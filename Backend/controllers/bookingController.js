...
export const createBooking = async (req, res) => {
    ...
    try {
        ...
        if (typeof car === "string" && /^[0-9a-fA-F]{24}$/.test(car)) {
            const carDoc = await carModel.findById(car).session(session).lean();
            ...
            const carExists = await carModel.exists({ _id: carSummary.id }).session(session);
        ...
        const createdArr = await bookingModel.create([bookingData], { session });
        ...
        await carModel.findByIdAndUpdate(carId, { $puch: { bookings: bookingEntry } }, { session })
        await session.commitTransaction();
        session.endSession();
        const saved = await bookingModel.findById(createBooking._id).lean();
